 ...
 */
async function createReviewForReferral(
  referralRequestId,
  requesterUserId,
  { stars, review_text },
) {
  ...
  const text =
    review_text != null && typeof review_text === "string"
      ? review_text.trim() || null
      : null;

  const pool = await getPool();

  const refResult = await pool
    .request()
  ...
  const existing = await pool
    ...
    .input("given_by_user_id", sql.BigInt, userId).query(`
  ...
  try {
    ...
    await reqTx
      ...
      .input("review_text", sql.NVarChar(2000), text).query(`
    ...
    await reqTx
      .input("provider_user_id", sql.BigInt, providerUserId)
      .input("stars", sql.TinyInt, s).query(`
        UPDATE users
        SET
          provider_rating =
            (
...
async function createReview(
  ...
  const completedReferral = await pool
    .request()
    .input("provider_user_id", sql.BigInt, providerUserId)
    .input("requester_user_id", sql.BigInt, givenByUserId).query(`
  ...
  const existing = await pool
    .request()
    .input("provider_user_id", sql.BigInt, providerUserId)
    .input("given_by_user_id", sql.BigInt, givenByUserId).query(`
  ...
  try {
    ...
    const createdResult = await reqTx
      ...
      .input("review_text", sql.NVarChar(2000), review_text).query(`
    ...
    const aggResult = await reqTx.input(
      "provider_user_id",
      sql.BigInt,
      providerUserId,
    ).query(`
    ...
    await reqTx
      ...
      .input("provider_rating_count", sql.Int, agg.count_stars).query(`
        UPDATE users
